...
insert into policies (name, category, description, severity, natural_language_rule, locked)
values
  ('Filesystem destruction', 'System safety', 'Requires approval before recursive local deletion or project/workspace removal.', 'critical', 'Ask before allowing rm -rf /, rm -rf ., recursive deletion of the current project/workspace, deleting the project directory, or formatting local disks/volumes.', true),
  ('Database destructive changes', 'Databases', 'Requires approval before destructive database commands.', 'critical', 'Ask before allowing DROP DATABASE, DROP TABLE, DROP SCHEMA, TRUNCATE TABLE, or unfiltered DELETE FROM statements.', true),
  ('Database mass update', 'Databases', 'Requires approval before broad database mutation.', 'high', 'Ask before allowing UPDATE statements that modify a table without an explicit WHERE clause.', true),
  ('Cloud resource deletion', 'Infrastructure', 'Requires approval before deleting cloud resources.', 'critical', 'Ask before deleting S3 buckets, GCP projects, Kubernetes namespaces, VMs, DNS zones, CloudFormation stacks, storage accounts, or similar cloud resources.', true),
  ('Terraform and Kubernetes destruction', 'Infrastructure', 'Requires approval before destructive infrastructure operations.', 'critical', 'Ask before terraform destroy, terraform apply -destroy, tofu destroy, kubectl delete namespace, helm uninstall, or equivalent destructive infrastructure operations.', true),
  ('Git commit or push', 'Source control', 'Requires approval before publishing or committing code.', 'medium', 'Ask before agents run git commit, git push, gh repo sync, release upload, or otherwise publish commits without approval.', false),
  ('Protected branch push', 'Source control', 'Requires approval before direct pushes to protected branches.', 'high', 'Ask before direct git pushes to main, master, trunk, production, prod, or release branches.', true),
  ('Git history rewrite or cleanup', 'Source control', 'Requires approval before destructive source-control operations.', 'high', 'Ask before git push --force, git reset --hard, git clean -fdx, interactive rebase rewrites, git filter-branch, or similar history/worktree destructive commands.', true),
  ('Committing secrets', 'Secrets and credentials', 'Requires approval before committing staged secrets.', 'critical', 'Ask before committing staged content that appears to include .env values, private keys, access tokens, API keys, cloud credentials, or similar secrets.', true),
  ('Dependency or lockfile changes', 'Supply chain', 'Requires approval before supply-chain changes.', 'medium', 'Ask before installing dependencies, upgrading packages, or modifying package-lock.json, pnpm-lock.yaml, yarn.lock, requirements.txt, poetry.lock, Cargo.lock, go.sum, .csproj, or similar manifests/lockfiles.', false),
  ('Global package install', 'Supply chain', 'Requires approval before global package installation.', 'medium', 'Ask before globally installing packages with npm, pnpm, yarn, pip, gem, cargo, go install, or similar package managers.', true),
  ('Secrets and credentials access', 'Secrets and credentials', 'Requires approval before touching secrets or credential stores.', 'critical', 'Ask before reading, copying, printing, editing, exfiltrating, or summarizing .env files, SSH keys, cloud credentials, API tokens, browser cookies, kubeconfigs, npm tokens, or password stores.', true),
  ('Personal data use', 'Data protection', 'Requires approval before using personal or regulated data.', 'high', 'Ask before processing personal, customer, employee, passport, SSN, credit card, or similarly regulated data.', false),
  ('External data sharing', 'Data protection', 'Requires approval before sending project data externally.', 'high', 'Ask before uploading files, calling unknown external URLs, pasting logs to third-party services, sending source code, or exfiltrating secrets during debugging.', true)
on conflict do nothing;

update policies set category = case
  when name in ('Filesystem destruction') then 'System safety'
  when name in ('Database destructive changes', 'Database mass update') then 'Databases'
  when name in ('Cloud resource deletion', 'Terraform and Kubernetes destruction') then 'Infrastructure'
  when name in ('Git commit or push', 'Protected branch push', 'Git history rewrite or cleanup') then 'Source control'
  when name in ('Committing secrets', 'Secrets and credentials access') then 'Secrets and credentials'
  when name in ('Dependency or lockfile changes', 'Global package install') then 'Supply chain'
  when name in ('Personal data use', 'External data sharing') then 'Data protection'
  else category
end;
